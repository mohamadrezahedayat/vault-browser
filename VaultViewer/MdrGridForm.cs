...
namespace VaultViewer
{
    public partial class MdrGridForm : Form
    {
        ...
        private void btnSyncDocDb_Click(object sender, EventArgs e)
        {
            ...
            //Insert Transmital for first coloumns
            for (int i = 0; i < filteredarray.GetUpperBound(0); i++)
            {


                var transmitalNumber = filteredarray[i, 9];
                var doublicate = db.TransmitalRipository.GetById(transmitalNumber);
                var days = filteredarray[i, 10];
                if (transmitalNumber != null && days.ToString() != "" && doublicate == null)
                {
                    ...
                    var transmitalDb = new DataLayer.Models.Transmital()
                    {
                        ...
                        POI = filteredarray[i, 7].ToString(),
            ...
            //Insert Transmital for second coloumns
            for (int i = 0; i < filteredarray.GetUpperBound(0); i++)
            {


                var transmitalNumber = filteredarray[i, 19];
                var doublicate = db.TransmitalRipository.GetById(transmitalNumber);
                var days = filteredarray[i, 20];
                if (transmitalNumber != null && days.ToString() != "" && doublicate == null)
                {
                    var date = new DateTime(1900, 01, 01).AddDays(Convert.ToDouble(days) - 2);
                    ...
                    var transmitalDb = new DataLayer.Models.Transmital()
                    {
                        ...
                        Date = date,
                        TransmitalNumber = transmitalNumber.ToString(),
                        POI = filteredarray[i, 17].ToString(),
...
// Insert Documents
            ...
            for (int i = 0; i < filteredarray.GetUpperBound(0); i++)
            {
                var doublicate = db.DocumentRipository.GetById(filteredarray[i, 0]);
                var transmitalNumber = "No Transmital";
                
                if (filteredarray[i, 9] != null && filteredarray[i, 9].ToString() != "")
                {
                    transmitalNumber = filteredarray[i, 9].ToString();
                }

                if (filteredarray[i, 0] != null && doublicate == null)
                {
                    var docDb = new DataLayer.Models.Document()
                    {
                        Title = filteredarray[i, 1].ToString(),
                        DocumentNumber = filteredarray[i, 0].ToString(),
